...
class Competence(Enum):
    ...
    def from_str(cls, value: str) -> Competence:
        ...
        """
        return next(filter(lambda o: o.name.lower() == value,
                           Competence))
...
class AnnotationTopic(Enum):
    ...
    SYNTAX_SENTENCE_ERROR = ('Períodos compostos mal estruturados '
                             'sintaticamente')
...
class CorpusMecEf:
    ...
    def _essay_from_entry(self, entry: dict) -> Essay:
        consolidated = self._competences_dict_to_enum_dict(
            entry[self._KEY_COMPETENCES])
        ...
        for k in annotations:
            ...
            annotator_competences = {
                c: annotator_entry[c.name.lower()]
                for c in Competence
            }

            # Obtendo anotações a nível de caracteres
            annotations_entry = annotator_entry[self._KEY_ANNOTATION]
            annots = [self._annotation_from_entry(e, text)
                      for e in annotations_entry]

            # Armazenando dados desse anotador
            annotations[k] = AnnotatorData(competences=annotator_competences,
                                           annotations=annots)

        return Essay(text=text,
    ...
    def _annotation_from_entry(self,
                               entry: list,
                               parent_text: str) -> Annotation:
        text = None
        ...
        return Annotation(topic=topic,
                          start_idx=start,
                          end_idx=end,
                          text_excerpt=text)

    def _competences_dict_to_enum_dict(self,
                                       d: dict) -> dict[Competence, int]:
        return {
            Competence.from_str(k): v
            for k, v in d.items()
        }
