...
class WordMorphosyntacticInformationExtractor(FeatureExtractor):
    ...
    def _compute_morphosyntactic_adjectives(self, doc: Doc) -> dict[str, float]:
        ...
        adjective_features["adjective_ratio"] = (
            len(all_adjectives) / len(all_tokens) if len(all_tokens) != 0 else 0
        )
    ...
    def _compute_morphosyntactic_nouns(self, doc: Doc) -> dict[str, float]:
        ...
        nouns_features["nouns_ratio"] = (
            len(all_nouns) / len(all_tokens) if len(all_tokens) != 0 else 0
        )
    ...
    def _compute_morphosyntactic_verbs(self, doc: Doc) -> dict[str, float]:
        ...
        infinitive_verbs = [
            token.text for token in all_verbs if token.morph.get("VerbForm") == ["Inf"]
        ]
        inflected_verbs = [
            token.text for token in all_verbs if token.morph.get("VerbForm") == ["Fin"]
        ]
        non_inflected_verbs = [
            token.text for token in all_verbs if token.morph.get("VerbForm") != ["Fin"]
        ]
        indicative_condition = [
            token.text for token in all_verbs if token.morph.get("Mood") == ["Cnd"]
        ]
        indicative_future = [
            token.text for token in all_verbs if token.morph.get("Tense") == ["Fut"]
        ]
        ...
        verbs_features["non_inflected_verbs"] = len(non_inflected_verbs) / len(
            all_verbs
        )
        ...
        if len(inflected_verbs) > 0:
            ...
            verbs_features["indicative_future_ratio"] = len(indicative_future) / len(
                inflected_verbs
            )
        ...
        for sentence in sentences:
            tokens_sentences = [token.text for token in sentence if not token.is_punct]
            verbs_sentence = [
                token.text for token in sentence if token.pos_ in verbs_pos_tags
            ]
    ...
    def _compute_prepositions_per_clause_sentence(self, doc: Doc) -> dict[str, float]:
        ...
        if len(all_clauses) > 0:
            features_preposition["prepositions_per_clause"] = total_prepositions / len(
                all_clauses
            )
            features_preposition["prepositions_per_sentence"] = (
                total_prepositions / len(sentences)
            )
    ...
    def _compute_morphosyntactic_pronouns(self, doc: Doc) -> dict[str, float]:
        ...
        all_pronouns_indefinite = [
            token.text for token in all_tokens if token.morph.get("PronType") == ["Ind"]
        ]
        all_pronouns_relative = [
            token.text for token in all_tokens if token.morph.get("PronType") == ["Rel"]
        ]
        ...
        features_pronouns["personal_pronouns"] = len(all_personal_pronouns) / len(
            all_tokens
        )
        features_pronouns["oblique_pronouns_ratio"] = len(all_pronouns_oblique) / len(
            all_pronouns
        )
        features_pronouns["indefinite_pronoun_ratio"] = len(
            all_pronouns_indefinite
        ) / len(all_pronouns)
        features_pronouns["relative_pronouns_ratio"] = len(all_pronouns_relative) / len(
            all_pronouns
        )
        ...
        for sentence in sentences:
            ...
            pronouns_ratio_sent = (
                len(pronouns_sentence) / len(tokens_sentences)
                if len(pronouns_sentence) > 0
                else 0
            )
    ...
    def _compute_morphosyntactic_other_metrics(self, doc: Doc) -> dict[str, float]:
        ...
        total_content_words = len(
            [
                token.text
                for token in doc
                if not token.is_punct and token.pos_ in content_words_pos_tags
            ]
        )
        total_functional_words = len(
            [
                token.text
                for token in doc
                if not token.is_punct and token.pos_ not in content_words_pos_tags
            ]
        )
        ...
        if total_content_words > 0:
            features["ratio_function_to_content_words"] = (
                total_functional_words / total_content_words
            )
