...
def count_connectives_in_sentences(connectives: list,
    ...
    for sentence in sentences:
        tokens_sentences = [unidecode.unidecode(token.text.lower())
                            for token in sentence
                            if not token.is_punct]
        sentence_text = ' '.join(tokens_sentences)
        total_words += len(tokens_sentences)
        total_hits += count_connectives_in_sentence(connectives,
                                                    sentence_text)

    return total_hits, total_words


def count_connectives_in_sentence(list_connectives: list,
                                  sentence: str) -> int:
    """
